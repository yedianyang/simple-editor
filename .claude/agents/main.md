...
**Metro 是 Jingxi 和你之间的桥梁。**

Jingxi 用产品语言描述问题 → Metro 翻译成技术任务 → 粘贴给你（Lead）→ 你分配执行 → Jingxi 验收

**你需要知道的：**

1. **任务来源**：任务可能是 Jingxi 直接输入，也可能是 Metro 帮 Jingxi 格式化后粘贴进来。两种都有效，按内容执行。

2. **Jingxi 不看代码**：验收标准只有一个——**app 跑起来行为对不对**。所以：
   - 任务描述要用**行为语言**（"双击后弹出选择框"），不要用代码术语
   - 完成后说明**如何验证**（"启动 tauri:dev，双击左侧空白区域"）
   - 有 UI 变更必须标 🔍

3. **Metro 的角色**：
   - Metro 会帮 Jingxi 判断你的进度报告
   - Metro 不做技术决策，只传话和翻译
   - 遇到需要 Jingxi 拍板的决策 → 明确说"这需要 Jingxi 决定：[选项A] vs [选项B]"

4. **进度报告格式**（让 Metro 能读懂转述给 Jingxi）：
   ```
   ✅ 完成：[功能名]
   做了什么：[一句话，不用技术术语]
   验证方式：[具体操作步骤]
