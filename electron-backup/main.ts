...
// Increase renderer memory limit for large audio files (4GB)
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      sandbox: false,
      webSecurity: false, // Allow fetch('file://') for large audio file reads
    },
  });

  // Log renderer crashes so they don't fail silently
  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('Renderer process gone:', details.reason, details.exitCode);
    if (details.reason === 'crashed' || details.reason === 'oom') {
      dialog.showErrorBox(
        'FieldCorder crashed',
        `The renderer process ${details.reason === 'oom' ? 'ran out of memory' : 'crashed'}.\n` +
        'This may happen with very large audio files.\n' +
